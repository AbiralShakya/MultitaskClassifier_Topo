...
def train_main_classifier():
    ...
    criterion_topology = EnhancedTopologicalLoss(
    alpha=config.LOSS_WEIGHT_TOPOLOGY,
    beta=config.LOSS_WEIGHT_TOPO_CONSISTENCY,
    gamma=config.LOSS_WEIGHT_REGULARIZATION
            ).to(config.DEVICE)
    criterion_topology.classification_loss = nn.CrossEntropyLoss(
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            loss_topology = criterion_topology(
                ...
                topological_features=topo_feats
            )
            loss_magnetism = criterion_magnetism(
                magnetism_logits,
                batch['magnetism_label']
            )
