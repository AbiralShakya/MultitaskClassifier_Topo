...
class BrillouinZoneEncoder(nn.Module):
    ...
    def __init__(self, hidden_dim: int = 128):
        ...
        self.symmetry_point_embedding = nn.Embedding(10, hidden_dim)  # Max 10 symmetry points
...
class BerryCurvatureLayer(MessagePassing):
    ...
    def __init__(self, in_channels: int, out_channels: int):
        ...
        self.berry_connection_net = nn.Sequential(
            nn.Linear(2 * in_channels + 3, out_channels),  # +3 for k-space distance vector
...
class PhysicsInformedKSpaceEncoder(nn.Module):
    ...
    def __init__(self, 
        ...
        self.berry_layers = nn.ModuleList([
            BerryCurvatureLayer(hidden_dim, hidden_dim) 
        ...
        self.topo_extractor = TopologicalInvariantExtractor(hidden_dim, hidden_dim // 2)
    ...
    def forward(self, data: PyGData) -> torch.Tensor:
        ...
        k_emb = self.bz_encoder(
            k_pos, 
            getattr(data, 'symmetry_labels', None)
        )
...
class EnhancedKSpacePhysicsFeatures(nn.Module):
    ...
    def __init__(self, 
        ...
        self.dos_encoder = nn.Sequential(
            nn.Linear(100, 64),  # Assuming 100-point DOS
            nn.LayerNorm(64),
            nn.ReLU(),
            nn.Linear(64, 32)
        )
        ...
        self.fermi_encoder = nn.Sequential(
            nn.Linear(50, 32),  # Fermi surface descriptors
            nn.LayerNorm(32),
            nn.ReLU(),
            nn.Linear(32, 16)
        )
        ...
        self.final_proj = nn.Sequential(
            nn.Linear(32 + 32 + 32 + 16, output_dim),  # Sum of all feature dims
            nn.LayerNorm(output_dim),
            nn.ReLU(),
            nn.Dropout(0.1)
        )
    ...
    def forward(self, 
        ...
        # Process optional features with fallback
        if gap_features is not None:
            gap_emb = self.gap_analyzer(gap_features)
        else:
            gap_emb = torch.zeros(decomp_emb.size(0), 32, device=decomp_emb.device)
            
        if dos_features is not None:
            dos_emb = self.dos_encoder(dos_features)
        else:
            dos_emb = torch.zeros(decomp_emb.size(0), 32, device=decomp_emb.device)
            
        if fermi_features is not None:
            fermi_emb = self.fermi_encoder(fermi_features)
        else:
            fermi_emb = torch.zeros(decomp_emb.size(0), 16, device=decomp_emb.device)
