...
class ImprovedAttentionFusion(nn.Module):
    ...
    def forward(self, features_list):
        # Project each modality to consistent feature space
        projected_features = []
        for features, proj in zip(features_list, self.modality_projections):
            projected = proj(features)
            projected_features.append(projected)
...
class HybridTopoClassifier(nn.Module):
    def __init__(
        ...
        # Binary classifier (trivial vs topological)
        self.classifier = nn.Sequential(
            nn.Linear(128, 256),
            nn.BatchNorm1d(256),
            nn.ReLU(),
        ...
        # Modality-specific classifiers for auxiliary binary losses
        self.cgcnn_classifier  = nn.Linear(128, 2)
        self.asph_classifier   = nn.Linear(128, 2)
        self.kspace_classifier = nn.Linear(128, 2)
    ...
    def forward(
        ...
        asph_out = self.asph(asph_features)
        kspace_gnn_out = self.kspace_gnn(
            kspace_graph.x,
            kspace_graph.edge_index,
            kspace_graph.edge_attr,
            kspace_graph.batch
        )
        kspace_transformer_out = self.kspace_transformer(
            kspace_graph.x,
            kspace_graph.edge_index,
            kspace_graph.edge_attr,
            kspace_graph.batch
