...
BAND_GAP_SCALAR_DIM = 1 # Single scalar band gap value
DOS_FEATURE_DIM = 100   # Assuming a 100-point DOS vector if available
FERMI_FEATURE_DIM = 1  # Assuming 50 descriptors for Fermi surface if available
...
# # NEW: Combined Magnetic-Topological Classes Mapping
# COMBINED_CLASS_MAPPING = {
#     ("Trivial", "NM"): 0,
#     ("Topological Insulator", "NM"): 1,
#     ("Semimetal", "NM"): 2,
...
# LEARNING_RATE = 0.001
# BATCH_SIZE = 8
# NUM_EPOCHS = 75 # Increased epochs to align with your output's "Epoch 1/75"
# DROPOUT_RATE = 0.2
# PATIENCE = 10 # For early stopping
# MAX_GRAD_NORM = 3.0 # Max norm for gradient clipping

# EGNN_HIDDEN_IRREPS_STR = "64x0e + 32x1o + 16x2e" # As defined in model.py's RealSpaceEGNNEncoder
# EGNN_RADIUS = 4.0 # Atomic interaction radius for EGNN

# # KSpace GNN specific parameters
# KSPACE_GNN_NUM_HEADS = 8 # As defined in model.py's KSpaceTransformerGNNEncoder

# # Loss weighting for multi-task learning
# LOSS_WEIGHT_PRIMARY_COMBINED = 1.0 # New weight for the combined task
# LOSS_WEIGHT_AUX_TOPOLOGY = 2.0   # α: weight on classification term for auxiliary topology
# LOSS_WEIGHT_TOPO_CONSISTENCY = 1.0  # β: weight on topological‐consistency term (within EnhancedTopologicalLoss)
# LOSS_WEIGHT_REGULARIZATION = 1.0  # γ: weight on topo‐feature regularization term (within EnhancedTopologicalLoss)
# LOSS_WEIGHT_AUX_MAGNETISM = 0.1  # weight on the magnetism classification term
...
# # K-space Decomposition Features (input to EnhancedKSpacePhysicsFeatures)
# BASE_DECOMPOSITION_FEATURE_DIM = 2 # From your existing code
# ALL_POSSIBLE_IRREPS = [] # This will be loaded from file, but provide a default for safety
# MAX_DECOMPOSITION_INDICES_LEN = 100 # From your existing code

# try:
#     # Adjust this path if the irrep_unique file is elsewhere
#     with open('/scratch/gpfs/as0714/graph_vector_topological_insulator/multitask_ti_classification/irrep_unique', 'rb') as fp:
#         ALL_POSSIBLE_IRREPS = pickle.load(fp)
# except FileNotFoundError:
#     warnings.warn("irrep_unique file not found. ALL_POSSIBLE_IRREPS will be empty. Decomposition features might be impacted.")
#     # Provide a small dummy list for ALL_POSSIBLE_IRREPS if file not found, to avoid calculation errors
#     ALL_POSSIBLE_IRREPS = ['A1', 'E', 'T1'] 
...
# # New feature dimensions for EnhancedKSpacePhysicsFeatures
# BAND_GAP_SCALAR_DIM = 1 # Single scalar band gap value
# DOS_FEATURE_DIM = 500   # Assuming a 100-point DOS vector if available
# FERMI_FEATURE_DIM = 50  # Assuming 50 descriptors for Fermi surface if available
...
# # --- Crystal Encoder Specific Parameters (ADDED THESE) ---
# # These parameters are directly used by TopologicalCrystalEncoder.
# # Ensure they align with your TopologicalCrystalEncoder's __init__ defaults or desired values.
# crystal_encoder_hidden_dim: int = 256 
# crystal_encoder_num_layers: int = 6
# crystal_encoder_output_dim: int = 128 # The main embedding output size of TopologicalCrystalEncoder
# crystal_encoder_radius: float = 4.0 # Should match the radius for MultiScaleEGNNConv
# crystal_encoder_num_scales: int = 3
# crystal_encoder_use_topological_features: bool = True # Crucial for enabling topo_extractor output
